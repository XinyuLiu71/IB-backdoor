...
parser.add_argument('--poison_rate', type=float, default=0.25, help='poison rate')
...
def create_datasets():
    ...
    total_class0_samples = len(training_data_npy['arr_0'][training_data_npy['arr_1'] == 0])
    poison_samples = int(total_class0_samples * args.poison_rate)
    ...
    for cls in args.observe_classes:
        ...
        if cls == 0:
            poison_data_num = int(len(observe_data_npy) * 0.25)
    ...
    return train_dataset, test_dataset, sample_datasets, backdoor_dataset, cls0_clean_dataset

def write_datasets(train_dataset=None, test_dataset=None, sample_datasets=None, backdoor_dataset=None, cls0_clean_dataset=None):
...
if __name__ == "__main__":
    ...
    train_dataset, test_dataset, sample_datasets, backdoor_dataset, cls0_clean_dataset = create_datasets()
    if args.dataset == 'all':
        write_datasets(train_dataset, test_dataset, sample_datasets, backdoor_dataset, cls0_clean_dataset)
