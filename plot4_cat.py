...
def plot_mi(MI_dict, mi_type, args, epochs):
    ...
    for class_idx, mi_values in MI_dict.items():
        ...
        plt.plot(epochs_to_plot, mi_estimates_to_plot, label=label, linestyle=linestyle, marker=marker)
    ...
    plt.savefig(os.path.join(output_dir, f'mi_{mi_type}_plot_cat.png'))
    plt.close()
...
def main(args):
    ...
    MI_inputs_vs_outputs2 = np.load(f"{args.directory2}/infoNCE_MI_I(X,T).npy", allow_pickle=True).item()
    MI_Y_vs_outputs2 = np.load(f"{args.directory2}/infoNCE_MI_I(Y,T).npy", allow_pickle=True).item()
