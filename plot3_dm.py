...
def generate_epochs_from_files(directory):
    epochs = []
    pattern = re.compile(r'mi_inputs-vs-outputs_class_comparison_(\d+)\.png')
...
def plot_mi(MI_dict, mi_type, args, epochs):
    ...
    for class_idx, mi_values in MI_dict.items():
        # 设置标签和样式
        if class_idx in ['class_0_sample', 'class_0_backdoor', 'class_0_clean']:
            # 对于backdoor和clean样本，使用虚线
            label = f'Class 0 {"Backdoor" if "backdoor" in class_idx else "Clean" if "clean" in class_idx else "Sample"}'
            linestyle = '--'
            if class_idx == 'class_0_backdoor':
                color = 'red'
            elif class_idx == 'class_0_clean':
                color = 'green'
            else:
                color = 'blue'
        else:
        ...
        plt.plot(epochs_to_plot, mi_estimates_to_plot, label=label, linestyle=linestyle, color=color)
...
def main(args):
    ...
    MI_inputs_vs_outputs = np.load(f"{directory}/mi_inputs_vs_outputs_all_dict.npy", allow_pickle=True).item()
    MI_Y_vs_outputs = np.load(f"{directory}/mi_Y_vs_outputs_all_dict.npy", allow_pickle=True).item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", type=str, default="ddpm-cifar10-label_consistent-0.1/up_blocks[-2]", help="Directory containing the data files")
