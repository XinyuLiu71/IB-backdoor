...
def train_loop(dataloader, model, loss_fn, optimizer, num_classes):
    ...
    return avg_acc, class_losses, t, labels_all, is_backdoor_all
...
def estimate_mi_wrapper(args):
    ...
    sample_batch_size = 90 if flag == "inputs-vs-outputs" else 512
...
def train(args, flag='inputs-vs-outputs', mode='infoNCE'):
    ...
    test_poison_data = np.load("data/wanet/0.01/poisoned_test_data.npz")
    ...
    for epoch in range(1, epochs + 1):
        print(f"------------------------------- Epoch {epoch} -------------------------------")
        train_acc, class_losses, t, labels, is_backdoor = train_loop(train_dataloader, model, loss_fn, optimizer, num_classes)
        ...
        if should_compute_mi:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=len(args.observe_classes)) as executor:
                # 计算 I(X,T) 和 I(T,Y)
                compute_args = [(args, 'inputs-vs-outputs', model_state_dict, class_idx, 350, mode) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, choices=['resnet18', 'vgg16'], default='vgg16', help='model architecture')
    ...
    parser.add_argument('--noise_std_xt', type=float, default=0.6, help='noise_std_xt')
