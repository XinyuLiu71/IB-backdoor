...
def estimate_mi(config, device, flag, model_state_dict, sample_loader, class_idx, mode='infoNCE'):
    ...
    elif flag == 'inputs-vs-predictions':
        Y_dim, Z_dim = 10, 3072 # Y's dimension, X's dimension
        initial_lr = config.mi_estimation.initial_lr
        epochs = config.mi_estimation.epochs
        num_negative_samples = config.mi_estimation.num_negative_samples
    else:
...
def train(args, config, flag='inputs-vs-outputs', mode='infoNCE'):
    ...
    MI_inputs_vs_predictions = {class_idx: [] for class_idx in config.observe_classes}
    class_losses_list = []
    ...
    for epoch in range(1, epochs + 1):
        ...
        if should_compute_mi:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=len(config.observe_classes)) as executor:    
                compute_args = [(config, 'inputs-vs-predictions', model_state_dict, class_idx, mode) 
                                 for class_idx in config.observe_classes]
                results_inputs_vs_predictions = list(executor.map(estimate_mi_wrapper, compute_args))
            ...
            for class_idx, result in zip(config.observe_classes, results_inputs_vs_predictions):
                mi_inputs_vs_predictions = result
                mi_inputs_vs_predictions_dict[class_idx] = mi_inputs_vs_predictions
                MI_inputs_vs_predictions[class_idx].append(mi_inputs_vs_predictions)
            ...
            plot_and_save_mi(mi_inputs_vs_predictions_dict, 'inputs-vs-predictions', args.outputs_dir, epoch)
            ...
            np.save(f'{args.outputs_dir}/infoNCE_MI_I(Y,T).npy', MI_inputs_vs_predictions)
            ...
            wandb.log({
                ...
                f"I(Y;T)_estimation": wandb.Image(os.path.join(args.outputs_dir, f'mi_plot_inputs-vs-predictions_epoch_{epoch}.png'))
            }, step=epoch)
    ...
    wandb.finish()
    return MI_inputs_vs_predictions, best_model
...
def ob_infoNCE(args):
    ...
    # )
    infoNCE_MI_log_inputs_vs_predictions, best_model = train(
        args, default_config, 'inputs-vs-predictions', 'infoNCE'
    )
    ...
    # 检查并保存 infoNCE_MI_log_inputs_vs_predictions
    infoNCE_MI_log_inputs_vs_predictions = np.array(infoNCE_MI_log_inputs_vs_predictions, dtype=object)
    np.save(f'{outputs_dir}/infoNCE_MI_I(Y,T).npy', infoNCE_MI_log_inputs_vs_predictions)
    print(f'saved in {outputs_dir}/infoNCE_MI_I(Y,T).npy')
