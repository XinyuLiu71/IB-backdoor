...
# train_data = np.load("data/badnet_/0.1/badnet_10%.npz")
# train_images = torch.tensor(train_data['arr_0'], dtype=torch.float32).permute(0, 3, 1, 2).to(device)
# train_labels = torch.tensor(train_data['arr_1'], dtype=torch.long).to(device)
image_pipeline = [
        ToTensor(), 
        ToDevice(device)
    ]
...
test_labels = torch.argmax(torch.tensor(torch.cat(test_labels)), dim=1).unsqueeze(1)
...
def imshow_grid(images, labels, title):
    ...
    for i, ax in enumerate(axes.flat):
        if i < num_images:
            ...
            ax.axis('off')
    plt.savefig(f"sample1%_class{labels[0].item()}")
